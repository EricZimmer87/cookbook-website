...
function IngredientsView() {
  ...
  useErrorRedirect(ingredientsError);

  const navigate = useNavigate();
  const location = useLocation();

  if (ingredientsLoading) return <p>Loading...</p>;
  if (ingredientsError) return <p>Error loading tags...</p>;
  ...
  return (
    <div>
      ...
      <AddButton
        onClick={() => navigate('/ingredients/new', { state: { from: location.pathname } })}
      />
      <table>
        <thead>
          <tr>
            ...
            .map((ingredient) => (
              <tr key={ingredient.ingredientId}>
                ...
                <td>
                  <EditButton
                    onClick={() =>
                      navigate(`/ingredients/${ingredient.ingredientId}/edit`, {
                  ...
                  <DeleteButton
                    onClick={() =>
                      navigate(`/ingredients/${ingredient.ingredientId}/delete`, {
                        state: { from: location.pathname },
                      })
                    }
                  />
